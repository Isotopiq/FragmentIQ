...
import { Button, Card, Progress, Select } from "flowbite-react";
...
export function JobMonitor() {
  ...
  useEffect(() => {
    if (!selected) return;
    api.jobLogs(selected.id).then((value) => setLogs(value.content));
            ...
            <ol className="space-y-3 border-l border-slate-200 pl-4">
              {stages.map((stage) => (
                <li key={stage} className="relative">
                  <span className={`absolute -left-[22px] top-1 h-3 w-3 rounded-full ${selected?.stage === stage ? "bg-blue-600" : "bg-slate-300"}`} />
                  <p className={selected?.stage === stage ? "font-semibold text-blue-700" : "font-medium text-slate-700"}>{stage}</p>
                  <p className="text-sm text-slate-500">{selected?.stage === stage ? "Active stage" : "Pipeline checkpoint"}</p>
                </li>
              ))}
            </ol>
